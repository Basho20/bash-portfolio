...
export default function Navbar() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-obsidian/92 backdrop-blur-md border-b border-obsidian/[0.07]">
        <div className="max-w-6xl mx-auto px-6 xl:px-10 h-[68px] flex items-center justify-between">

          {/* Wordmark */}
          <Link href="/" className="group flex items-baseline gap-2.5">
            <span className="f-display font-black text-xl tracking-tight text-ivory group-hover:text-copper transition-colors duration-300" style={{ fontVariantLigatures: 'none' }}>
              MB
            </span>
            ...
            {links.slice(1).map(l => (
              <Link key={l.href} href={l.href}
                className={`group relative f-body text-[11px] tracking-wide font-medium transition-colors duration-300
                  ${pathname === l.href ? 'text-copper' : 'text-slate hover:text-ivory'}`}>
                {l.label}
      ...
      {/* Mobile overlay */}
      <div className={`fixed inset-0 z-40 bg-obsidian flex flex-col justify-center px-8 transition-all duration-500
        ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="mb-10">
          <span className="f-display text-6xl font-black text-ivory/10">Menu</span>
        </div>
          ...
          {links.map((l, i) => (
            <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
              className={`group flex items-center justify-between py-5 transition-colors duration-200
                ${pathname === l.href ? 'text-copper' : 'text-ivory hover:text-copper'}`}>
